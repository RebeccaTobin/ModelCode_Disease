...
std::string SetRawName(int NNodes = Parameters::N) {
    ...
    + "nd" + std::to_string(nd) +"Alpha"+ to_string(alpha) +"AvgDeg"+std::to_string(avgdeg) + MortalityCondition + Topology + "SingleSeed" + std::to_string(SingleSeed) + NetworkType + "pA" + to_string(p_assortativity) + "tau" + to_string(tau) + "repair_k" + std::to_string(repair_k);
}
