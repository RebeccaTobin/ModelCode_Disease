...
std::string SetRawName(int NNodes = Parameters::N) {
    ...
    + "nd" + std::to_string(nd) +"Alpha"+ to_string(alpha) +"AvgDeg"+std::to_string(avgdeg) + MortalityCondition + Topology + "SingleSeed" + std::to_string(SingleSeed) + NetworkType + "pA" + to_string(p_assortativity)+"RateIncrease"+to_string(RateIncrease) +
        "StartTime"+to_string(StartTime)+"tau"+to_string(tau) +
